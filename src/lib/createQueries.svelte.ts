...
import { createState, onCleanup } from './utils.svelte.js';
...
export function createQueries<T extends Array<any>, TCombinedResult = QueriesResults<T>>(
	...
	return untrack(() => {
		...
		const state = createState(getInitialResult());
		...
		$effect(() => {
			...
			if (isFrozen()) {
				options = {
					...options,
					queries: options.queries.map((queryOption) => {
						return { ...queryOption, enabled: false };
					}),
				};
			}
			...
			if (init) {
				untrack(() => {
					return observer.setQueries(options.queries, options as QueriesObserverOptions<TCombinedResult>);
				});
			}
		...
		onCleanup(
			observer.subscribe(
				createEventHandler(() => {
					const $defaultedOptions = defaultedOptions.value;
